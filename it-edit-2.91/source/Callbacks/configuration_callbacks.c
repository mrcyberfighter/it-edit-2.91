...
inline void apply_configuration_change(GKeyFile *conf_file) {
  ...
  g_key_file_set_value(conf_file,   "Terminal", "command",               settings.command) ;
  ...
  g_key_file_set_value(conf_file,   "Terminal", "cursor_color",          settings.cursor_color)  ;
  g_key_file_set_value(conf_file,   "Terminal", "bold_color",            settings.bold_color)  ;
...
void write_changes_to_app_file(void) {
  ...
  if (error != NULL) {
    ...
    fprintf(stderr, _("Cannot save conf file:\n%s\n"), error->message) ;
